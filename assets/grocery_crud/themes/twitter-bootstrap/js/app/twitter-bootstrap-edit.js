$(function(){
    ...
    $('.return-to-list').on('click', function() {
        ...
        goToList('Confirmation', message_alert_edit_form,$(this).data('target-url'));
...
function backToList(list_url)
{
    window.location = list_url;
}

//  Retornar para a tabela de listagem de dados inicial
function goToList(title_modal, message_text,list_url){

    if ($('#dialog_modal_message')[0]){
        $('#dialog_modal_message').remove();
    }

    var modal_content = '<div id="dialog_modal_message" class="modal hide-table fade" tabindex="-1" role="dialog" aria-labelledby="dialog_modal_message_label" aria-hidden="true"><div class="modal-header">  <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>  <h3 id="dialog_modal_message_label">' + title_modal + '</h3></div><div class="modal-body">  <p>'+ message_text + '</p></div><div class="modal-footer">  <button class="btn cancel-confirmation" data-dismiss="modal" aria-hidden="true">Cancel</button> <a onclick="javascript:backToList(\''+list_url+'\')" class="btn btn-dinarpal ok-confirmation">Ok</a></div></div>';
