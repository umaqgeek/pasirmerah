...
function padam(title_modal, message_text,delete_url){
	if (confirm(message_text) == true) {
        deteleGroceryCrudInformation(delete_url);
    } else {
        return false;
    }
}
...
function removemodal(){
	...
	$('a.cancel-confirmation').trigger('click');
}

function deleteModalConfirm(delete_url){
	
deteleGroceryCrudInformation(delete_url);
removemodal();
}
//	Chama o método para excluir o registro de informação do BD
function confirmationModalDialog(title_modal, message_text,delete_url){


	if ($('#dialog_modal_message')[0]){
		$('#dialog_modal_message').remove();
	}
	var modal_content = '<div id="dialog_modal_message" class="modal hide-table fade" tabindex="-1" role="dialog" aria-labelledby="dialog_modal_message_label" aria-hidden="true"><div class="modal-header">	<a class="close" data-dismiss="modal" aria-hidden="true">×</a>	<h3 id="dialog_modal_message_label">' + title_modal + '</h3></div><div class="modal-body">	<p>'+ message_text + '</p></div><div class="modal-footer">	<a onclick="javascript:removemodal()" class="btn cancel-confirmation" data-dismiss="modal" aria-hidden="true">Cancel</a><a id="'+delete_url+'" onclick="javascript:deleteModalConfirm(\''+delete_url+'\')" class="btn btn-dinarpal ok-confirmation">Ok</a></div></div>';
	
	$('#ajax_list').before(modal_content);


	//console.log($(this).data('target-url'));

	$('.ok-confirmation').click(function(){
		//alert('opoawe');
		//deteleGroceryCrudInformation(delete_url);
		//removemodal();
